...
class Tab extends CWP_P_FW{
     ...
     */
    function __construct($name, $description, $icon = '', Page $parent_page, $main_tab = false, $slug=''){
        $this->parent_page = $parent_page;
        $this->name = $name;
        $this->description = $description;
        $this->icon = $icon;
        
      /*  $this->slug = sanitize_title('cwp_login' . $name, 'Default Tab Name');*/
        $this->slug = sanitize_title('cwp_login' . $slug, 'Default Tab Name');
        ...
        add_action( 'cwp_login_admin_tabs_links_' . $this->parent_page->get_slug() , array( $this , 'render_tab_link' ) );
        ...
        if( ( !isset( $_GET[ 'tab' ] ) && $main_tab ) || ( isset( $_GET[ 'tab' ] ) && $_GET['tab'] == $this->slug ) ) {
            ...
            add_action('cwp_login_admin_tabs_contents_' . $this->parent_page->get_slug(), array($this, 'render_tab_content'));
        }
    ...
    public function render_tab_content(){
        echo "<div class='tab-content cwp-tab-" . esc_attr($this->slug) . "'><form action='options.php' method='post'>";
