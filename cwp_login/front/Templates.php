...
class Templates extends Front
{
    ...
    function __construct($selector = '')
    {
        ...
        if ( $this->formTemplate && $this->formTemplate !== 'default' ) {
            self::$body_classes[] = 'clt-login-form-template-' . $this->formTemplate;
        }
    ...
    function template_enabled_notice(){
        if( !$this->restoreNow ) {
            ...
            echo "<div class='update-nag notice is-dismissible'><p>登录模板切换成功!
                    <a href='$uri'>撤销切换</a></p>
                </div>";
        }else{
            echo "<div class='updated'><p>已撤销回上一次的登录模板！如效果不理想,可再次切换!</p></div>";
        }
    }
